...
<div class="row" id="row-form">
	<div class="col-sm-12" style="margin-bottom:5px "><h2>Tambah Pengguna</h2><hr></div>
	<div class="col-sm-2"></div>
	<div class="col-sm-8">
		@foreach($hakAkses as $key)
		<form method="POST" action="{{asset('proses-tambahPengguna/'. $key->menuid)}}">
			
			<input type="hidden" id="_token" name="_token" value="{{csrf_token()}}">
			
			<div class="form-group {{ $errors->has('namadepan') ? 'has-danger' : ''}}">
				<label for="namadepan">Nama Depan</label>
				<input type="text" class="form-control" id="namadepan" name="namadepan" value="{{old('namadepan')}}">
				<small><div class="form-control-feedback">{!! $errors->first('namadepan',':message') !!}</div></small>
			</div>
			
			<div class="form-group">
				<label for="namabelakang">Nama Belakang</label>
				<input type="text" class="form-control" id="namabelakang" name="namabelakang" value="{{old('namabelakang')}}">
			</div>
			
			<div class="form-group {{ $errors->has('email') ? 'has-danger' : ''}}">
				<label for="email">Email</label>
				<input type="text" class="form-control" id="email" placeholder="nama@contoh.com" name="email" value="{{old('email')}}">
			...
			<div class="form-group {{ $errors->has('peran') ? 'has-danger' : ''}}">
				...
				<select class="form-control combobox" id="peran" name="peran">
					...
					<option value="{{$row->id}}" {{old('peran') == $row->id ? 'selected' : ''}}>{{$row->nama}}</option>
					@endforeach
				</select>
